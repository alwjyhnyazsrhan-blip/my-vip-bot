...
# --- 1. الإعدادات وتنسيق التصميم (CSS) ---
st.set_page_config(page_title="VIP ACCESS", page_icon="👑")

st.markdown("""
    <style>
    .stApp {background-color: #050505 !important;}
    .vip-card {background-color: #050505; padding: 20px; border-radius: 15px; text-align: center; color: white;}
    .stTextInput > div > div > input {
        background-color: #1a1a1a !important; color: #888 !important; 
        border: 1px solid #333 !important; border-radius: 10px !important; 
        text-align: center; height: 50px;
    }
    div.stButton > button[kind="primary"] {
        background-color: #00ff88 !important; color: black !important; 
...
st.markdown("<h1>👑</h1><p>الوصول المميز</p><h2>VIP ACCESS</h2>", unsafe_allow_html=True)
...
if not st.session_state.authenticated:
    ...
    if st.button("تسجيل الدخول", type="primary"):
        # مفتاح التفعيل (يجب أن يطابق ما ستعطيه للمستخدمين)
    ...
    st.button("⚙️ نسخ المعرف")
    st.button("💎 شراء كود VIP")

else:
    # --- 3. لوحة تحكم البوت (تظهر فقط بعد تسجيل الدخول) ---
